from classes import MenuKeyboard, MembershipKeyboard, SupportKeyboard, BackKeyboard, PaymentKeyboard, MenuKeyboardNoLimit, EndKeyboard
...
async def get_photo1(call):
    ...
    if user.balance > 0:
        ...
        users_states[user_id] = ['WAITING_FOR_PHOTO', call.message.message_id]
    else:
        await bot.edit_message_caption(chat_id=user_id, message_id=call.message.message_id,
...
@bot.message_handler(func=lambda message: users_states[message.from_user.id][0] == 'WAITING_FOR_PHOTO')
async def get_photo2(message):
    user_id = message.from_user.id
    end_keyboard = EndKeyboard()
    if message.content_type == 'photo':
        photo = message.photo[-1].file_id
        await bot.edit_message_reply_markup(chat_id=user_id, message_id=users_states[user_id][1], reply_markup=None)
        await rate_photo(message, photo)
    else:
        check_message = await bot.send_message(chat_id=user_id, reply_markup=end_keyboard.markup,
                                text='🍷 Пришлите фото или отмените оценку внешности по кнопке ниже')
        await bot.edit_message_reply_markup(chat_id=user_id, message_id=users_states[user_id][1], reply_markup=None)
        users_states[user_id].append(message.message_id)
        users_states[user_id].append(check_message.message_id)


@bot.callback_query_handler(func=lambda call: call.data.startswith('cancel_photo'))
async def cancel_photo(call):
    user_id = call.from_user.id
    await bot.delete_message(users_states[user_id][3])
    await bot.delete_message(users_states[user_id][2])
    await create_menu(user_id=user_id, mode=True, message=users_states[user_id][1])
...
async def rate_photo(message, photo):
    user_id = message.from_user.id
    async_client = genai.client.AsyncClient(api_key=key)
    file_info = await bot.get_file(photo)
    image_bytes = await bot.download_file(file_info.file_path)
    analysis_message = await bot.send_message(user_id, text='🔍 Анализ фотографии . . .')
    ...
    await bot.send_photo(user_id, photo=photo, caption='🍷 Результат анализа фотографии:')
    await bot.send_message(user_id, text=answer)
